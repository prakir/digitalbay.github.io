...
class EventTimelineSet extends _typedEventEmitter.TypedEventEmitter {
  ...
  insertEventIntoTimeline(event, timeline, roomState) {
    ...
    if (this.room && !this.canContain(event)) {
      ...
      _logger.logger.warn(`EventTimelineSet.insertEventIntoTimeline: Ignoring ${eventDebugString} that does not belong ` + `in timeline=${timeline.toString()} timelineSet(threadId=${(_this$thread4 = this.thread) === null || _this$thread4 === void 0 ? void 0 : _this$thread4.id})`);
      return;
    }
  ...
  canContain(event) {
    ...
    if (!shouldLiveInRoom && !shouldLiveInThread) {
      var _this$room2;
      _logger.logger.warn(`EventTimelineSet:canContain event encountered which cannot be added to any timeline roomId=${(_this$room2 = this.room) === null || _this$room2 === void 0 ? void 0 : _this$room2.roomId} eventId=${event.getId()} threadId=${event.threadRootId}`);
    }
