...
class FetchHttpApi {
  ...
  async tryRefreshToken() {
    ...
    try {
      ...
      (_this$opts$logger = this.opts.logger) === null || _this$opts$logger === void 0 || _this$opts$logger.warn("Failed to refresh token", error);
      return false;
    }
  }
  ...
  async requestOtherUrl(method, url, body, opts = {}) {
    var _this$opts$logger2, _opts$json, _body$constructor, _opts$localTimeoutMs, _opts$keepAlive;
    const urlForLogs = this.sanitizeUrlForLogs(url);
    (_this$opts$logger2 = this.opts.logger) === null || _this$opts$logger2 === void 0 || _this$opts$logger2.debug(`FetchHttpApi: --> ${method} ${urlForLogs}`);
    ...
    const jsonBody = json && (body === null || body === void 0 || (_body$constructor = body.constructor) === null || _body$constructor === void 0 ? void 0 : _body$constructor.name) === Object.name;
    if (json) {
    ...
    try {
      ...
      (_this$opts$logger3 = this.opts.logger) === null || _this$opts$logger3 === void 0 || _this$opts$logger3.debug(`FetchHttpApi: <-- ${method} ${urlForLogs} [${Date.now() - start}ms ${res.status}]`);
    } catch (e) {
      var _this$opts$logger4;
      (_this$opts$logger4 = this.opts.logger) === null || _this$opts$logger4 === void 0 || _this$opts$logger4.debug(`FetchHttpApi: <-- ${method} ${urlForLogs} [${Date.now() - start}ms ${e}]`);
